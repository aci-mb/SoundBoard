...
namespace AcillatemSoundBoard.Tests.ViewModel
{
    ...
    public class SoundContextMenuCommandsTests
    {
        ...
        private static SoundContextMenuCommands CreateTargetWithDefaultStubs(IMainWindowViewModel mainWindowViewModel = null, IDialogService dialogService = null, IKernel container = null)
        {
            if (container == null)
            {
            ...
            return new SoundContextMenuCommands(
        ...
        {
            Target.ActivateSoundsCommand.CanExecute(new Collection<ISound> {MockRepository.GenerateStub<ISound>(), MockRepository.GenerateStub<ISound>()})
                .Should().BeTrue();
        }
        ...
        public void ActivateSoundsCommandExecute_ParameterContainsIListWith3Sounds_Adds3EquivalentClonesToActiveSounds()
        {
            ...
            stub.ActiveSounds = activeSounds;

            Target = CreateTargetWithDefaultStubs(mainWindowViewModel: stub);
            var selectedSounds = new List<ISound>
            {
                StubSoundWithRandomName(),
				StubSoundWithRandomName(),
				StubSoundWithRandomName()
            };
            ...
            {
                activeSounds[i].ShouldBeEquivalentTo(selectedSounds[i], "for each selected sound there must be an identical active sound");
                activeSounds[i].Should().NotBeSameAs(selectedSounds[i], "it must be a clone, not the same reference");
            }
        }
        ...
        {
            Target.RenameSoundCommand.CanExecute(new Collection<ISound> { MockRepository.GenerateStub<ISound>(), MockRepository.GenerateStub<ISound>() })
        ...
        public void RenameSoundCommandExecute_DialogReturnsNotNull_NameOfSelectedSoundEqualsGivenName()
        {
            ...
            ISound selectedSound = StubSoundWithName("Old Name");
        ...
        public void RemoveSoundsCommandCanExecute_SoundBoardIsNotNullAndParameterIsIListWith3Sounds_ReturnsTrue()
        {
            ...
            Target.RemoveSoundsCommand.CanExecute(new Collection<ISound> {MockRepository.GenerateStub<ISound>(), MockRepository.GenerateStub<ISound>(), MockRepository.GenerateStub<ISound>()})
                .Should().BeTrue();
        }
